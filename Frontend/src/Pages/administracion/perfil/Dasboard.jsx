...
const Dashboard = ({ empleado }) => {
  const { miCargo, obtenerMiCargo, obtenerMiCargoPorId } = useAuthEmpleado();
  useEffect(() => {
    if (empleado) {
      obtenerMiCargoPorId(empleado.idEmpleado);
    } else {
      obtenerMiCargo();
    }
  }, []);
        ...
        <div className="flex flex-wrap justify-center mt-20 pb-20 ">
          ...
          <div className="bg-white rounded-3xl shadow-xl">
            <div className=" px-4 text-center mt-1">
              <div className="flex justify-center py-1 lg:pt-4 pt-8"></div>
              <div className="text-center mt-1">
                ...
                <div className="mb-2 text-blueGray-600 mt-5">
                  ...
                  {miCargo?.unidad?.nombre} {/* Nombre de la unidad */}
                </div>
                <div className="mb-2 text-blueGray-600 mt-2">
                  <i className="fas fa-briefcase mr-2 text-lg text-blueGray-400 "></i>
                  <span className="font-semibold">Cargo:</span>{" "}
                  {miCargo?.cargo?.nombre} {/* Nombre del cargo */}
                </div>
                <div className="mb-2 text-blueGray-600 mt-2 max-w-[275px]">
                  <i className="fas fa-university mr-2 text-lg text-blueGray-400"></i>
                  <span className="font-semibold">Descripción del cargo:</span>{" "}
                  {miCargo?.cargo?.descripcion}
